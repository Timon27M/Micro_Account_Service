...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleHttpMessageNotReadableException(
        ...
        log.error(ex.getMessage());
        ErrorResponse error = new ErrorResponse(HttpStatus.FORBIDDEN, "Переданы не валидные данные");
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
    }
    ...
    public ResponseEntity<ErrorResponse> handleAuthorizationDenied(AuthorizationDeniedException ex) {
        log.error(ex.getMessage());
        ErrorResponse error = new ErrorResponse(HttpStatus.FORBIDDEN, "Access Denied: Admin role required");
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
    }
    ...
    public ResponseEntity<ErrorResponse> handleGeneralException(Exception ex) {
        log.error(ex.getMessage());
        ErrorResponse error = new ErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR, ex.getMessage() != null ? ex.getMessage() : "Внутренняя ошибка сервера");
    ...
    public ResponseEntity<ErrorResponse> handleResponseStatusException(ResponseStatusException ex) {
        log.error(ex.getMessage());
        ErrorResponse error = new ErrorResponse(ex.getStatusCode(), ex.getReason());
